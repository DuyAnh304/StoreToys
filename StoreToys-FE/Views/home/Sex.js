...
const productUrl = 'http://localhost:8080/StoreToys-API/product/sex/';
...
function start(){
    ...
    getBrand();
    if (sex) { 
      getProductBySex(decodeURIComponent(sex)); 
  } else {
      getProduct();
  };
    getQuantityCart();
...
function getProductBySex(sex) {
    let urlWithName = `${productUrl}${sex}`;
    fetch(urlWithName)
    .then(function(res) {
      return res.json();
    })
    .then(function(datas) {
      var filteredProducts = datas.data;
...
function renderProduct(data){
    if(sex === "nam"){
        categoryTitle.textContent = "Bé trai";
    } else if(sex === "nu"){
        categoryTitle.textContent = "Bé gái";
    } else{
        categoryTitle.textContent = "Unisex";
    }
    const productList = document.querySelector('.product-list');
