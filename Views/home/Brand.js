...
const productUrl = 'http://localhost:8080/StoreToys-API/product/brand/';
...
function start(){
    getCategory();
    getBrand();
    getBrandByName(decodeURIComponent(brandName));
    if (brandName) {
...
function getBrandByName(name){
    let urlWithName = `${brandUrl}name/${name}`;
    fetch(urlWithName)
    .then(function(res){
        return res.json();
    })
    .then(function(datas){
        brandImg.src = `${imgUrl}${datas.data[0].brand_img}`;
    })
    .catch(error => console.log(error));
}

function getProductByBrand(brandName) {
    let urlWithName = `${productUrl}${brandName}`;
    fetch(urlWithName)
    .then(function(res) {
      return res.json();
    })
